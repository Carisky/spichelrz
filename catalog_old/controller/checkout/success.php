...
class ControllerCheckoutSuccess extends Controller
{
	public function index()
	{
		...
		if (isset($this->session->data['order_id'])) {
			...
			// //< Noir
			// $this->load->controller('crm/crm/order', $this->session->data['last_order_id']);
			...
			// 		'points' => round((float)$order['total']/20, 2)
			// 	];
			// 	$this->model_checkout_order->addRewards($data);
			// }
			// // Noir >
		}
		...
		*/
		...
		if ($this->customer->isLogged() && !empty($this->session->data['last_order_id'])) {
			$data['text_message'] = sprintf($this->language->get('text_customer'), $this->url->link('account/order/info&order_id=' . $this->session->data['last_order_id'], '', true), $this->url->link('account/account', '', true), $this->url->link('account/order', '', true), '/kontakt', $this->url->link('product/special'), $this->session->data['last_order_id'], $this->url->link('account/download', '', true));
		} else {
			$data['text_message'] = sprintf($this->language->get('text_guest'), '/kontakt');
		}
