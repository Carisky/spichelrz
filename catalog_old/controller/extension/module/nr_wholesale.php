...
class ControllerExtensionModuleNrWholesale extends Controller
{
        ...
        if ($this->customer->getGroupId() != 2) {
            ...
            foreach ($categories as &$category) {
                ...
                foreach ($products as &$product) { //
                    ...
                    $product['old_price_currency'] = $this->currency->format(round($product['old_price'] * (1 - floatval($this->config->get('module_nr_wholesale_discount')) / 100), 2),$this->session->data['currency']);
                    $product['price_currency'] = $this->currency->format($price, $this->session->data['currency']);
    ...
    protected function validateAddress($address, $products)
    {
        ...
        if ((utf8_strlen(trim($address['nip'])) < 1) || (utf8_strlen(trim($address['nip'])) > 10))
