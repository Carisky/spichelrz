		...
		if ($category_info) {
			...
			foreach ($results as $result) {
				...
				if ($result['meta_description']) {
					$schema['description'] = $result['meta_description'];
				}
				if ($result['mpn']) {
					$schema['mpn'] = $result['mpn'];
				}
				if ($result['sku']) {
					$schema['sku'] = $result['sku'];
				}
				if ($result['ean']) {
					$schema['gtin'] = $result['ean'];
				}
				if ($result['model']) {
				...
				if ($result['special']) {
					$schema['offers']['price'] = $this->tax->calculate($result['special'], $result['tax_class_id'], $this->config->get('config_tax'));
				} else {
					$schema['offers']['price'] = $this->tax->calculate($result['price'], $result['tax_class_id'], $this->config->get('config_tax'));
				}
				...
				if ($result['special_date_end']) {
					$schema['offers']['priceValidUntil'] = $result['special_date_end'];
				} else {
					$schema['offers']['priceValidUntil'] = date('Y-m-d');
				}
				if ((int)$result['reviews'] > 0) {
					$schema['aggregateRating']['@type'] = 'AggregateRating';
					$schema['aggregateRating']['ratingValue'] = (int)$result['rating'];
					$schema['aggregateRating']['reviewCount'] = (int)$result['reviews'];
				}
				if ($result['manufacturer']) {
					$schema['brand']['@type'] = 'Brand';
					$schema['brand']['name'] = $result['manufacturer'];
					$schema['brand']['url'] = $this->url->link('product/manufacturer/info', 'manufacturer_id=' . $result['manufacturer_id']);
				}
				...
				$this->document->itemList = array_map(function($product) {
					return $product['href'];
				}, $data['products']);
				
				// перед рендером
				$data['schemas_org'] = $this->load->controller('common/schemas');
				...
				$data['products'][] = array(
					...
					'href'        => $this->url->link('product/product',  '&product_id=' . $result['product_id'] . $url) //'path=' . $this->request->get['path'] .
				);
			}
