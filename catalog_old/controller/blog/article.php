	...
	if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_tag'])) {
		...
		if ($article_info) {
			...
			if($article_info['image']) {
				$data['image'] = $this->model_tool_image->resize($article_info['image'], 1920, 'x');
				$data['image1'] = $this->model_tool_image->resize($article_info['image'], 1100, 'x');
				$data['image2'] = $this->model_tool_image->resize($article_info['image'], 700, 'x');
				$data['ratio'] = 1;
				try {
					if ($data['image']) {
						$sizes = getimagesize(DIR_IMAGE.explode('/image/', $data['image'])[1]);
						$data['ratio'] = $sizes[1]/$sizes[0];
					}
				} catch (\Throwable $th) {}
