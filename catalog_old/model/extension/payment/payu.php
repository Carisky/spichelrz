...
class ModelExtensionPaymentPayu extends Model
{
    public function getMethod($address, $total)
    {
        ...
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . intval($this->config->get('payment_payu_geo_zone_id')) . "' AND country_id = '" . intval($address['country_id']) . "' AND (zone_id = '" . intval($address['zone_id']) . "' OR zone_id = '0')");
