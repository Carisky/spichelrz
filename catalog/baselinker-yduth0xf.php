 ...
 */
$options['baselinker_pass'] = 'ji5p88tk0fr29laevjnh8kankd7lbn1c';		// communication password, provided by Baselinker on the connection form

$options['db_host'] = 'localhost';		// database host (commonly localhost)
$options['db_user'] = 'wrapxpl_spichu';		// database user
$options['db_pass'] = 'sarVhIY7TWO6';		// database password
$options['db_name'] = 'wrapxpl_spich';		// database name
$options['db_prefix'] = 'oc_';			// database table prefix, e.g. oc_, leave empty to detect automatically

$options['images_folder'] = 'https://spichlerz24.pl/image/';  // root URL for the images folder (end with a single slash)
...
$options['lang'] = 'pl'; // the language to be used for product translations (either code or name, exactly as it is displayed in opencart)
$options['want_invoice_cf'] = ''; // custom order field number indicating the need to generate an invoice, field_no:value, e.g. 1:2
$options['nip_cf'] = '3'; // custom order field number where buyer's tax ID is stored, e.g. 2
...
function Shop_FileVersion($request)
{
	...
	$response['version'] = "4.2.28"; //wersja pliku integracyjnego, nie wersja sklepu!
...
function Shop_OrdersGet($request)
{
	...
	while($order = DB_Fetch($res))
	{
		...
		// paczkomat
		if (empty($o['delivery_point_name']) and empty($o['delivery_point_id']) and preg_match('/^inpost.point_([\w\-]+)/', $order['shipping_code'], $m))
		{
			$o['delivery_point_id'] = $m[1];
			$o['delivery_point_name'] = $m[1];
		...
		if ($order['shipping_code'] == 'inpost.point') {
			...
			$o['delivery_point_city'] = (isset($pin[5])?$pin[5]:$pin[4]);
			$o['delivery_point_postcode'] = (isset($pin[5])?$pin[4]:"");
 ...
 * @global array $options : tablica z ustawieniami ogólnymi z początku pliku
 * @param array $request tablica z żadaniem od systemu, w przypadku tej funkcji nie używana
 * @return array $response tablica zawierająca dostępne sposoby płatności
 *		(id pobrane z konfiguracji sklepu, nazwy z tabeli zamówień)
 */
function Shop_PaymentMethodsList($request)
{
	...
	$response = array();
	$sql = "SELECT DISTINCT `code` FROM `{$dbp}setting` where `code` LIKE 'payment_%'";
	$res = DB_Query($sql);

	while ($method = DB_Fetch($res))
	{
		$response[substr($method['code'], 8)] = str_replace('_', ' ', substr($method['code'], 8));
	}

	// proper names from the orders table
	if (!empty($response))
	{
		$sql = "SELECT DISTINCT `payment_method` name, `payment_code` code
			FROM `{$dbp}order`";
		$res = DB_Query($sql);

		while ($method = DB_Fetch($res))
		{
			if (isset($response[$method['code']]) and !empty($method['name']))
			{
				$response[$method['code']] = $method['name'];
			}
		}
	}
 ...
 * @global array $options : tablica z ustawieniami ogólnymi z początku pliku
 * @param array $request tablica z żadaniem od systemu, w przypadku tej funkcji nie używana
 * @return array $response tablica zawierająca dostępne sposoby wysyłki:
