<?php
// Heading
$_['heading_title']                  = 'Kasa';
$_['heading_subtitle']               = 'Tradycyjne przetwory to wymarzony upominek zarówno dla smakoszy rodzimych, polskich smaków.';

// Text
$_['text_cart']                      = 'Mój koszyk';
$_['text_your_details']              = 'DANE ODBIORCY';
$_['text_shipping_method']           = 'DOSTAWA';
$_['text_payment_method']            = 'PŁATNOŚĆ';
$_['text_totals']            = 'PODSUMOWANIE';
$_['text_empty']       = 'Twój koszyk jest pusty!';
$_['text_success_coupon']  = 'Sukces: Zastosowano kupon rabatowy!';
$_['text_success_reward']  = 'Sukces: Zastosowano rabat na podstawie punktów lojalnościowych!';


$_['text_checkout_option']           = 'Krok 1: Opcje zamówienia';
$_['text_checkout_account']          = 'Krok 2: Konto i szczegóły płatności';
$_['text_checkout_payment_address']  = 'Krok 2: Szczegóły płatności';
$_['text_checkout_shipping_address'] = 'Krok 3: Szczegóły dostawy';
$_['text_checkout_shipping_method']  = 'Krok 4: Sposób dostawy';
$_['text_checkout_payment_method']   = 'Krok 5: Sposób zapłaty';
$_['text_checkout_confirm']          = 'Krok 6: Potwierdzenie zamówienia';
$_['text_modify']                    = 'Modyfikuj &raquo;';
$_['text_new_customer']              = 'Nowy klient';
$_['text_returning_customer']        = 'Zarejestrowany klient';
$_['text_checkout']                  = 'Opcje zamówienia:';
$_['text_i_am_returning_customer']   = 'Jestem klientem zarejestrowanym';
$_['text_register']                  = 'Załóż konto';
$_['text_guest']                     = 'Zamów jako gość';
$_['text_register_account']          = 'Zakładając konto, będziesz mógł kupować szybciej, być na bieżąco ze statusem zamówień oraz śledzić poprzednie zamówienia.';
$_['text_forgotten']                 = 'Zapomniałem hasło';
$_['text_discount'] = 'Rabat';
$_['text_total'] = 'Wszystko';

$_['text_your_address']              = 'Adres';
$_['text_your_password']             = 'Twoje hasło';
$_['text_agree']                     = 'Przeczytałem i zgadzam się z <a href="%s" class="agree"><b>%s</b></a>';
$_['text_address_new']               = 'Chcę użyć nowego adresu';
$_['text_address_existing']                   = 'Chcę użyć wcześniej wprowadzonego adresu';


$_['text_comments']                           = 'Dodaj komentarz na temat zamówienia';
$_['text_recurring']                          = 'Element cykliczny';
$_['text_payment_recurring']                  = 'Profil płatności';
$_['text_trial_description']                  = '%s każdego %d %s przez %d płatności a potem';
$_['text_payment_description']                = '%s każdego %d %s przez %d płatności';
$_['text_payment_until_canceled_description'] = '%s każdego %d %s do odwołania';
$_['text_day']                       = 'dzień';
$_['text_week']                      = 'tydzień';
$_['text_semi_month']                = 'pół miesiąca';
$_['text_month']                     = 'miesiąc';
$_['text_year']                      = 'rok';

// Column
$_['column_name']                    = 'Nazwa produktu';
$_['column_model']                   = 'Model';
$_['column_quantity']                = 'Ilość';
$_['column_price']                   = 'Cena jednostkowa';
$_['column_total']                   = 'Razem';

// Entry
$_['entry_email_address']            = 'Adres e-mail';
$_['entry_email']                    = 'E-mail';
$_['entry_password']                 = 'Hasło';
$_['entry_confirm']                  = 'Potwierdź hasło';
$_['entry_firstname']                = 'Imię';
$_['entry_lastname']                 = 'Nazwisko';
$_['entry_telephone']                = 'Telefon';
$_['entry_fax']                      = 'Faks';
$_['entry_address']                  = 'Wybierz adres';
$_['entry_company']                  = 'Firma';
$_['entry_customer_group']           = 'Grupa klientów';
$_['entry_address_1']                = 'Adres';
$_['entry_address_2']                = 'Adres cd.';
$_['entry_postcode']                 = 'Kod pocztowy';
$_['entry_city']                     = 'Miejscowość';
$_['entry_country']                  = 'Kraj';
$_['entry_zone']                     = 'Województwo';
$_['entry_newsletter']               = 'Chcę zapisać się do newslettera %s.';
$_['entry_shipping'] 	             = 'Mój adres dostawy i adres rozliczeniowy są takie same.';

// Error
$_['error_login']                    = 'Ostrzeżenie: E-mail i/lub hasło nieprawidłowe.';
$_['error_attempts']                 = 'Ostrzeżenie: Przekroczono dozwoloną liczbę prób logowań na Twoje konto. Spróbuj ponownie za 1 godzinę.';
$_['error_approved']                 = 'Ostrzeżenie: Twoje konto wymaga zatwierdzenia zanim będziesz mógł się zalogować.';
$_['error_exists']                   = 'Ostrzeżenie: Adres e-mail jest już zarejestrowany!';
$_['error_firstname']                = 'Imię musi mieć od 1 do 32 znaków!';
$_['error_lastname']                 = 'Nazwisko musi mieć od 1 do 32 znaków!';
$_['error_email']                    = 'Adres e-mail wygląda na nieprawidłowy!';
$_['error_telephone']                = 'Telefon musi mieć od 9 do 20 znaków!';
$_['error_password']                 = 'Hasło musi zawierać od 3 do 20 znaków!';
$_['error_confirm']                  = 'Potwierdzenie hasła nie pasuje do hasła!';
$_['error_address_1']                = 'Adres musi mieć od 3 do 128 znaków!';
$_['error_city']                     = 'Miejscowość musi mieć od 2 do 128 znaków!';
$_['error_postcode']                 = 'Kod pocztowy musi mieć od 2 do 10 znaków!';
$_['error_country']                  = 'Wybierz kraj!';
$_['error_zone']                     = 'Wybierz województwo!';
$_['error_agree']                    = 'Ostrzeżenie: Musisz wyrazić zgodę na %s!';
$_['error_address']                  = 'Ostrzeżenie: Musisz wybrać adres!';
$_['error_shipping']                 = 'Ostrzeżenie: Musisz wybrać sposób dostawy!';
$_['error_no_shipping']              = 'Ostrzeżenie: Brak dostępnych opcji dostawy. Prosimy o <a href="%s">kontakt z nami</a> w celu uzyskania pomocy!';
$_['error_payment']                  = 'Ostrzeżenie: Musisz wybrać sposób zapłaty!';
$_['error_no_payment']               = 'Ostrzeżenie: Brak dostępnych opcji zapłaty. Prosimy o <a href="%s">kontakt z nami</a> w celu uzyskania pomocy!';
$_['error_custom_field']             = '%s wymagane!';
$_['error_empty_coupon']             = 'Uwaga: Proszę wprowadzić kod kuponu!';
$_['error_coupon']  = 'Ostrzeżenie: Kupon jest nieprawidłowy, wygasł lub osiągnął limit użycia!';
$_['error_reward']  = 'Ostrzeżenie: Proszę podać ilość punktów lojalnościowych do korzystania!';
$_['error_points']  = 'Uwaga: Nie masz %s punktów lojalnościowych!';
$_['error_shipping_data'] = 'Ostrzeżenie: Dane do wysyłki wymagane!';
$_['error_empty_products']           = 'Uwaga: Nie wybrałeś produktów!';
$_['error_agree1']                   = 'Ostrzeżenie: Musisz zaakceptować Regulamin Sklepu Internetowego oraz Politykę Prywatności!';
$_['error_shipping_method'] = 'Proszę wybrać metodę wysyłki';
$_['error_nip'] = 'NIP musi zawierać tylko cyfry i mieć długość do 10 znaków.';

