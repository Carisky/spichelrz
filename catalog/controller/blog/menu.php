...
class ControllerBlogMenu extends Controller {
	...
	public function nr_menu() //Noir
	{
		$this->load->model('blog/category');
		$this->load->model('blog/article');
		$category = $this->model_blog_category->getCategories(0)[0];
		return [
			'title' => $category['name'],
			'href'  => $this->url->link('blog/category', 'blog_category_id='.$category['blog_category_id']),
			'active' => (!empty($this->request->get['blog_category_id']) and $this->request->get['blog_category_id'] == $category['blog_category_id'])
		];
	}
}