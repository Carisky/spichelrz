...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if (!empty($categories)) {
			...
			$category_id = $categories[0]['category_id']; // первая категория
		...
		if ($product_info) {
			...
			$data['review_action'] = $this->url->link(
			...
			$data['old_price'] = $this->currency->format($this->tax->calculate($product_info['old_price'], $result['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);
			...
			$data['price_numeric'] = $this->parsePrice($data['price']);

			$data['old_price_numeric'] = $this->parsePrice($data['old_price']);
...
public function review() {
    ...
    // Если пришёл POST — обрабатываем отправку
    if ($this->request->server['REQUEST_METHOD'] === 'POST') {
        $json = [];

	$last = isset($this->session->data['last_review_time'])
		? (int)$this->session->data['last_review_time']
		: 0;

	// 2) блокируем, если с последнего отзыва прошло меньше 15 секунд
	if ($last && time() < $last + 15) {
		$json['error'] = 'Poczekaj 15 sekund przed kolejnym wysłaniem.';
	}

        // 2) Проверка reCAPTCHA
        if (empty($json)) {
            $recaptcha = $this->request->post['g-recaptcha-response'] ?? '';
            $secret    = '6LcdvkYrAAAAAGL3x_n0s7X869Vjmlbx_PEMBnJQ';
            ...
            if (empty($result['success'])) {
                $json['error'] = 'Błąd weryfikacji reCAPTCHA.';
            }
        }

        // 3) Базовая валидация полей
        if (empty($json)) {
            if (empty($this->request->post['rating'])
                || empty($this->request->post['name'])
                || empty($this->request->post['text'])
            ) {
        ...
        if (empty($json)) {
            ...
            $this->session->data['last_review_time'] = time();
            $json['success'] = $this->language->get('text_success');
        }

        // 5) Отдаём JSON и завершаем
