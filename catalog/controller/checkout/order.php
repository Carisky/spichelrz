...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if(!empty($product['image'])) $this->db->query("UPDATE ".DB_PREFIX."order_product SET image = '".$this->db->escape($product['image'])."' WHERE order_product_id = ".$order_product_id); // Noir
				if(!empty($product['sku'])) $this->db->query("UPDATE ".DB_PREFIX."order_product SET sku = '".$this->db->escape($product['sku'])."' WHERE order_product_id = ".$order_product_id); // Noir
				if(!empty($product['weight'])) $this->db->query("UPDATE ".DB_PREFIX."order_product SET weight = '".$this->db->escape($product['weight'])."' WHERE order_product_id = ".$order_product_id); // Noir
			}
		}
	...
	public function addRewards($data) //Noir
	{
		$this->db->query("
			INSERT INTO ".DB_PREFIX."customer_reward SET 
