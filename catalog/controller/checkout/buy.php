...
class ControllerCheckoutBuy extends Controller {
	public function index() {
		...
		$this->document->setTitle("Mój koszyk");
	...
	public function address($address_id = 0)
	{
		...
		if (isset($this->session->data['shipping_address'])) {
			...
			if($data['logged']) {
				...
				$data['shipping_address']['firstname'] = $customer_info['firstname'];
				$data['shipping_address']['lastname'] = $customer_info['lastname'];
				$data['shipping_address']['company'] = $cost_fields[1];
				$data['shipping_address']['nip'] = $cost_fields[3];
				$data['shipping_address']['postcode'] = $cost_fields[7];
				$data['shipping_address']['city'] = $cost_fields[6];
				$data['shipping_address']['address_1'] = $cost_fields[4];
			}
		}
	...
	protected function calculateTotals()
	{
		...
		$data_totals = array(); $dd = 0; 
		foreach($total_data['totals'] as $row){
			if ($row['code']=="tax") $dd++;
			$data_totals[($row['code']=="tax"?$row['code'].$dd:$row['code'])] = $row;
		}
	...
	protected function saveOrder($post)
	{
		...
		if (empty($this->session->data['shipping_address'])) {
			...
			// $order_data['payment_tax_id'] =  isset($nip) ? $nip : '';
