	...
	protected function saveProducts($data)
	{
		...
		foreach($data as $key => $item) {
			...
			if($product_id) {
				...
				$url = $this->urlTransliterate($item['text_fields']['name']);
				...
				foreach($this->languages as $language) {
					...
					$product['product_seo_url'][0][$language['language_id']] = ($code != $language['code'] ? substr($language['code'], 0, 2).'-' : '').$url;
				}
				
				$categories = array_flip($this->schema['category_id']);
				$product['main_category_id'] = $categories[$item['category_id']];
