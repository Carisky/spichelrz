...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		//AG PRICE BRUTTO - NETTO
		$data['tax_rule'] = $this->model_localisation_tax_class->getTaxRules($data['tax_class_id']);

		$this->load->model('localisation/tax_rate'); 

		$data['tax_rate'] = 0.00;
		$data['price_brutto'] = 0.00;

		if (is_array($data['tax_rule'])) {
			if ($data['tax_rule'][0]['tax_rate_id']!=0) {
				$tax_rate = $this->model_localisation_tax_rate->getTaxRate($data['tax_rule'][0]['tax_rate_id']) ;
				$data['tax_rate'] = floatval($tax_rate['rate']);
				$data['price_brutto'] = round( ((int)$data['price']!=0?(float)$data['price']:0) * (1 + ($data['tax_rate']/100)) , 4) ;		
			}
		}
		//AG END
